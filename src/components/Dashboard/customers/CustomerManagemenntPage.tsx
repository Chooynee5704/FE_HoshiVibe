...
export default function CustomerManagementPage({
  ...
  const customers = Array(6).fill(null).map((_, i) => ({
    id: i + 1,
    cid: "CUST-" + (1000 + i),
    name: "Nguyen Thi Anh",
                ...
                <thead className="bg-gray-50 border-b border-gray-200">
                  <tr>
                    <th className="px-4 py-3 w-12">
                      <Checkbox checked={selectedCustomers.length === customers.length} onChange={toggleAllCustomers} />
                    </th>
                ...
                <tbody className="divide-y divide-gray-200">
                  {customers.map((customer) => (
                    <tr key={customer.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-4">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 bg-gray-200 rounded-full overflow-hidden">
                            <img src={customer.avatar || "/placeholder.svg"} alt={customer.name} className="w-full h-full object-cover" />
                          </div>
                          <button
                            className="text-sm font-medium text-blue-600 hover:underline"
                            onClick={() => onOpenDetail?.(customer.cid)}
                      ...
                      <td className="px-4 py-4"><span className="text-sm text-gray-800">{customer.email}</span></td>
                      <td className="px-4 py-4"><span className="text-sm text-gray-800">{customer.phone}</span></td>
                      <td className="px-4 py-4"><span className="text-sm text-gray-800">{customer.address}</span></td>
                      <td className="px-4 py-4"><span className="text-sm text-gray-800">{customer.joinedDate}</span></td>
                      <td className="px-4 py-4">
                        <button
